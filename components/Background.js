...
const LinkedInLogo = () => {
  const texture = useLoader(TextureLoader, "linkedIn_logo.png");
  const boxRef = useRef();
  useFrame(() => {
    if (!boxRef.current) {
      return;
    ...
    if (boxRef.current.rotation.x >= 2 * Math.PI) {
      boxRef.current.rotation.x = 0;
    }
    boxRef.current.rotation.y += 0.01;
    if (boxRef.current.rotation.y >= 2 * Math.PI) {
  ...
  return (
    <mesh
      ...
      position={[-2, 3.6, -2]}
      onPointerEnter={() => {
        boxRef.current.scale.y += 0.05;
        boxRef.current.scale.x += 0.05;
        boxRef.current.scale.z += 0.05;
      }}
      onPointerLeave={() => {
        boxRef.current.scale.y -= 0.05;
        boxRef.current.scale.x -= 0.05;
      ...
      onClick={() => {
        window.open("https://www.linkedin.com/in/gerhardus-cronje-7972221b1/");
      }}
    >
...
const GitHubLogo = () => {
  const texture = useLoader(TextureLoader, "github-mark-white.png");
  const boxRef = useRef();
  useFrame(() => {
    if (!boxRef.current) {
      return;
  ...
  return (
    <mesh
      ...
      onClick={() => {
        window.open("https://github.com/GerhardusC");
      }}
    >
      <boxGeometry attach="geometry" args={[1, 1, 1]} />
      <meshStandardMaterial attach="material" map={texture} color={"#fff"} />
    </mesh>
  );
};
